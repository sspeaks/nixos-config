# Opt-in SSH authorized keys for the `sspeaks` user.
#
# Kept out of ./default.nix so hosts that are never reached over SSH from these
# machines (WSL, VMs, ...) do not pick them up implicitly. Import this from any
# host you want to log into from sspeaks-pc, the MacBook, or Windows.
{ ... }:
{
  users.users.sspeaks.openssh.authorizedKeys.keys = [
    "ssh-rsa  sspeaks@sspeaks-pc"
    "ssh-rsa  sspeaks@seths-mbp.lan"
    "ssh-rsa  seth@sspeaks-pc-windows"
  ];
}
